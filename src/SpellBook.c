...
void Book_update(Entity* self);

int Book_collision(DynamicBody* self, List* collision);
...
Entity* Spellbook_New(Vector2D position, Vector2D gridPosition)
{
	...
	if (!ent)return NULL;
	ent->sprite = gf2d_sprite_load_all("images/Book.png",
		16,
		16,
		1,
		0);
	...
	ent->body.touch = Book_collision;
	ent->update = Book_update;
			...
			if (body)
			{
				...
				if (ent)
				{
					...
					if (ent->uniqueEntityTypeID == 1 && self->entityAttached->timer < 48.0) {
						...
						if (piperDataPointer->currentSpellBooks < piperDataPointer->maxSpellBooks) {
