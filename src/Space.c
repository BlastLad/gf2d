...
void gf2d_space_dynamic_bodies_step(Space* space, DynamicBody* db, float t)
{
    ...
    gf2d_space_dynamic_bodies_world_clip(space, db, t);//this is the dynamic body agianst world clipper
    if (db->blocked)
    {
        db->blocked = 0;
...
void gf2d_space_update_bodies(Space* space, float loops)
{
    ...
    for (i = 0; i < count; i++)
    {
        db = (DynamicBody*)gfc_list_get_nth(space->dynamicBodyList, i);
        if (!db)continue;
        gf2d_dynamic_body_update(db, loops);//updates individual body
    }
}
...
void gf2d_space_update(Space* space)
{
    ...
    Uint32 then;
    if (!space)return;
    gf2d_space_fix_overlaps(space, 8);//resolves if a dynamic body clips a static body
    gf2d_space_reset_bodies(space);//resetter
    // reset all body tracking

    //if (__DEBUG)then = SDL_GetTicks();
    for (s = 0; s <= 1; s += space->timeStep)
    {
        gf2d_space_step(space, s);//updates based on buckets or not
        loops = loops + 1;
    }
    //if (__DEBUG)slog("collision took %i milliseconds", SDL_GetTicks() - then);
    gf2d_space_update_bodies(space, loops);//updates all bodies
}
